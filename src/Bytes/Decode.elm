...
decode (Decoder decoder) bs =
    Native.Bytes.decode decoder bs
...
signedInt16 endianness =
    Decoder (Native.Bytes.read_i16 (endianness == LE))
...
signedInt32 endianness =
    Decoder (Native.Bytes.read_i32 (endianness == LE))
...
unsignedInt16 endianness =
    Decoder (Native.Bytes.read_u16 (endianness == LE))
...
unsignedInt32 endianness =
    Decoder (Native.Bytes.read_u32 (endianness == LE))
...
float32 endianness =
    Decoder (Native.Bytes.read_f32 (endianness == LE))
...
float64 endianness =
    Decoder (Native.Bytes.read_f64 (endianness == LE))
...
bytes n =
    Decoder (Native.Bytes.read_bytes n)
...
string n =
    Decoder (Native.Bytes.read_string n)
...
fail : Decoder a
fail =
    Decoder Native.Bytes.decodeFailure
